class number : AlignWidget, EventReceiver
{
	...
	public static number create(AlignWidget app) {
		var v = new number();
		v.app = app;
		return(v);
	}
	public override void initialize() {
		...
		animate(1000000);
		var black_1 = Color.instance("#000000");
		var font = Font.instance("5mm");	
		...
		//REVISIONS
		add_align(0, -0.8, y= LabelWidget.instance().set_text("Quiz#1").set_color(black_1).set_font(Font.instance("10mm")));
		add_align(0, -0.2, y= LabelWidget.instance().set_text("5 + 4 = ?").set_color(black_1).set_font(Font.instance("7mm")));
		vb.add(tw = TextInputWidget.instance());
		add_align(0, 0.4, bw= ButtonWidget.instance().set_text("OK").set_event("answer"));
		
		add_align(0.3, -1, y= LabelWidget.instance().set_text("Time:").set_color(black_1).set_font(font));
		add_align(0.6, -1,  y= LabelWidget.for_string("".append(String.for_integer(timer))).set_color(black_1).set_font(font));	
	}
		...
		if(o!=null) {		
			if(o is String) {
				...
				if(i.equals("main")) {
					...
					s.show(this, number2.create(AlignWidget.instance()), CrossFadeTransition.instance());			
				}				
